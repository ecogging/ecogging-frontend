...
export default function SavePlogging() {
  ...
  const savePloggingPage = () => {
    axios.post(`http://localhost:8080/myaccompaniestemp`,{userId:userId, page:page})
      .then((res) => {
  ...
  return (
    ...
    <div style={{margin: '0 100px'}}>
      <Link style={{margin:'20px', fontSize:'20px', fontWeight:'bold', color:'black'}} to={`/mypage/${userId}/plogging/savePlogging/1`} className="link_myPageTabMenu">모임</Link>  
      <Link style={{margin:'30px', fontSize:'20px', fontWeight:'bold', color:'lightgray'}} to={`/mypage/${userId}/plogging/saveEventPlogging/1`} className="link_myPageTabMenu">행사</Link>
    ...
    <Pagination aria-label="Page navigation example" style={{ margin: '0 auto', width: '900px', justifyContent: 'center', marginTop: '30px' }}>
        <PaginationItem disabled={curPage === 1}>
          <PaginationLink aria-label="Previous" href={`/mypage/${userId}/plogging/savePlogging/${curPage-1}`}>
        ...
        {pageBtn && pageBtn.map(item => {
          return (
            <PaginationItem className={item == curPage ? 'active1' : ''} key={item}>
              <PaginationLink id={item} href={`/mypage/${userId}/plogging/savePlogging/${item}`} >
                {item}
              </PaginationLink>
            </PaginationItem>
          );
        ...
        <PaginationItem disabled={curPage === totalPages}>
          <PaginationLink  aria-label="Next" href={`/mypage/${userId}/plogging/savePlogging/${curPage+1}`}>
            <span aria-hidden="true">›</span>
          </PaginationLink>
        </PaginationItem>
      </Pagination>   
