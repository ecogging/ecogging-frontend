...
export default function ParticipationPlogging() {
  ...
  useEffect(() => {
    ParticipationPloggingPage();
  }, []);

  const ParticipationPloggingPage = () => {
    axios
        .post(`http://localhost:8080/myparticipations`,{userId:userId, page:page})
        .then((res) => {
  ...
  return (
    ...
    <Pagination aria-label="Page navigation example" style={{ margin: '0 auto', width: '900px', justifyContent: 'center', marginTop: '30px' }}>
        ...
        <PaginationItem disabled={curPage === totalPages}>
          <PaginationLink  aria-label="Next" href={`/mypage/${userId}/plogging/participationPlogging/${curPage+1}`}>
            <span aria-hidden="true">›</span>
          </PaginationLink>
        </PaginationItem>
      </Pagination>   
